...
//----------store options-----------------
  ...
  time_store            =  1.E+5;                  //store after this time
...
//----------------------------------------------------------------------------------------------------
  ...
  log_mass_central   =  log10(1/c_grav);
...
void initial_conditions(void)
{
  ...
  switch (mode_initial_cond)
  {
    ...
    case 3:
      ...
      for (i = 1; i <= N; i++)
        sigma[i] = (3.0/(sqrt(tau0)*pow((4.0*c_pi),(3.0/2.0)))) * pow(radius[i],(-9.0/8.0)) * exp( - pow((1.0 - pow(radius[i],(3.0/4.0)) ),2)/tau0 ) + 1.E-10;

      for (i = 1; i <= N; i++)
      {
        u[1][i] = sigma[i]*radius[i];
        u[3][i] = u[1][i]*sqrt(c_grav*mass_central*radius[i]);
        u[2][i] = u[1][i] * (9.0/2.0) * beta * (radius[i]/tau0) * (1.0 - pow(radius[i],(-3.0/4.0)));
      }
